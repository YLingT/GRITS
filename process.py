...
def main():
    ...
    dir_path = "/media/yling/Space1/GRITS/dataset3/dataset"
    out_dir = "/media/yling/Space1/GRITS/dataset3/process_dataset2"
    ...
    for i in range(len(food_item)):
        for j in range(cfg.data.trial_num):
            ee_pose = np.load(dir_path + "/" + food_item[i] + "{}/ee_pose_qua.npy".format(str(j+1).zfill(2)))
    ...
    for i in range(len(food_item)):
        for j in range(cfg.data.trial_num):
            ...
            rgb_front_orin = np.load(dir_path + "/" + food_item[i] + "{}/rgb_front.npy".format(str(j+1).zfill(2)))
            depth_front_orin = np.load(dir_path + "/" + food_item[i] + "{}/depth_front.npy".format(str(j+1).zfill(2)))
            ee_pose_orin = np.load(dir_path + "/" + food_item[i] + "{}/ee_pose_qua.npy".format(str(j+1).zfill(2)))

            rgb_front = rgb_front_orin[10:260]
            depth_front = depth_front_orin[10:260]
            ee_pose = ee_pose_orin[10:260]

            assert rgb_front.shape[0]==250
            assert depth_front.shape[0]==250
            assert ee_pose.shape[0]==250
            ...
            assert rgb_front.shape[0]==250+To-1
            assert depth_front.shape[0]==250+To-1
            assert ee_pose_6d.shape[0]==250+To-1
